...
class IdmDfnSelector:
    ...
    def __init__(
        self,
        dfn_d: dict = None,
        outdir: Union[str, os.PathLike] = os.curdir,
        varnames: list = None,
    ):
        """IdmDfnSelector init"""
        ...
        assert self._outdir.is_dir(), f"Output directory does not exist: {outdir}"
    ...
    def _write_master(self):
        ofspec = self._outdir / "IdmDfnSelector.f90"
    ...
    def _write_selectors(self):
        for c in self._d:
            ofspec = self._outdir / f"Idm{c.title()}DfnSelector.f90"
            with open(ofspec, "w") as fh:
...
if __name__ == "__main__":
    ...
    # create definition selector files
    selectors = IdmDfnSelector(dfn_d=dfn_d, outdir=outdir)
    selectors.write()
