...
**Note**: A line providing the version number, date and DOI of the release, e.g. `6.4.4 & February 13, 2024 & \url{https://doi.org/10.5066/P9FL1JCC}`, is added to the Release History section of `ReleaseNotes.tex` automatically, using `doc/ReleaseNotes/release_history.py` (`pixi run add-release-history`). For an approved release this happens once, in the `prepare` job, before anything is built (see [Build assets/distributions](#build-assetsdistributions)); release candidate branches get no line, since `release_history.py` skips non-release version strings. The date is the date the workflow starts, which is also used for build timestamps and the software citation, unless the branch tip is already a prepared commit for the same version (a retry after a failed build), in which case its date is reused instead so the retry doesn't add a new bot commit. DOIs are updated with minor releases and remain the same for patch releases, so patch releases reuse the DOI of the minor release they patch.
...
For an approved release (not a dry run), a `prepare` job runs first: it applies the version and release history metadata (`update_version.py`, `release_history.py`, `fprettify`) once, commits it to the release branch, and pushes. The build, docs and dist jobs then check out that exact commit by SHA rather than the branch tip, and don't reapply the metadata themselves &mdash; so the binaries, documentation and distribution archives are all built from precisely the commit that later gets merged to `master`. If a build fails and the release is retried, `prepare` detects that the branch tip is already a prepared commit for the same version and reuses its date, so retries don't pile up extra bot commits.

If this is a dry run (branch name ends with `rc`), `prepare` is skipped entirely: binaries are built with `IDEVELOPMODE` set to 1, each job applies its own version metadata directly on the branch tip (as before this restructuring), no commit is pushed, no release history line is added, and the workflow ends after uploading artifacts for review. If this is not a dry run, the workflow will continue, drafting a pull request against the `master` branch from the release branch (whose tip is now the commit `prepare` pushed) after the build completes.
