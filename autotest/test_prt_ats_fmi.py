"""
Test a PRT model FMI-coupled to an ATS-enabled GWF model.
"""
...
def build_prt_sim(name, gwf_ws, prt_ws, mf6):
    from test_prt_fmi import build_prt_sim as _build_prt_sim

    sim = _build_prt_sim(name, gwf_ws, prt_ws, mf6)
    tdis = sim.get_package("tdis")
    tdis.perioddata = [
        (
            FlopyReadmeCase.perlen,
            2,  # to match TDIS produced by ATS for the flow model
...
def plot_output(idx, test):
    ...
    prt_pls = pd.read_csv(prt_ws / prt_track_csv_file, na_filter=False)
