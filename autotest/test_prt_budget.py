...
def check_output(idx, test):
    ...
    sto_in = next(
        iter([term[1] for term in prt_bud_cum if term[2].decode() == "STORAGE_IN"])
    )
    sto_out = next(
        iter([term[1] for term in prt_bud_cum if term[2].decode() == "STORAGE_OUT"])
    )
    term_in = next(
    ...
    assert prp_in > 0.0
    assert abs(term_out) > 0.0
    assert np.isclose(prp_out, 0.0)
    assert sto_in > 0.0
    assert abs(sto_out) > 0.0
    ...
    rls_flow = prp_bud[0]["q"]
    assert len(rls_flow) == nparticles
    # expect positive flow at release and none in all subsequent steps
    assert np.all(rls_flow > 0)
    for i in range(1, nstp):
        assert np.all(prp_bud[i]["q"] == 0)

    # check storage and termination terms
    sto_bud = prt_bud.get_data(text="STORAGE")
    trm_bud = prt_bud.get_data(text="TERMINATION")
    assert len(sto_bud) == nstp - 1  # no storage in first step
    assert len(trm_bud) == nstp - 1  # no termination in first step

    # storage should decrease with time
    last_sto_shp = sto_bud[0].shape[0]
    for i in range(1, nstp - 1):
        assert sto_bud[i].shape[0] <= last_sto_shp
        last_sto_shp = sto_bud[i].shape[0]

    # termination should increase with time
    last_trm_shp = trm_bud[0].shape[0]
    for i in range(1, nstp - 1):
        assert trm_bud[i].shape[0] >= last_trm_shp
        last_trm_shp = trm_bud[i].shape[0]
