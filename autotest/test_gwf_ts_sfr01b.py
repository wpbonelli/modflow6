"""
This is a modification of test_gwf_ts_sfr01.py that tests the use of
multiple time series by a boundary package (SFR). Currently, there is a lack
of autotests confirming that multiple time series files used by a single
boundary package is working correctly.  The test is simple, make sure the
time series inputs are generated and beyond that the built-in pytest
functionality will ensure the simulation ran to completion.
"""
...
def get_model(ws, name, timeseries=False):
    ...
    ts1_names = ["inflow", "divflow", "ustrf"]
    ts2_names = auxnames
    perioddata = [
        [0, "status", "active"],
        [1, "status", "active"],
    ...
    if timeseries:
        # in this variation of the autotest, attempting to implement 2 ts files
        ...
        ts1_methods = ["linearend"] * len(ts1_names)
        ts2_methods = ["linearend"] * len(ts2_names)
        ts1_data = []
        ts2_data = []
        for t in ts_times:
            ts1_data.append((t, inflow, divflow, upstream_fraction))
            ts2_data.append((t, temp, conc))
    else:
        perioddata.append([0, "inflow", inflow])
        perioddata.append([2, "diversion", 1, divflow])
    ...
    if timeseries:
        fname1 = f"{name}.sfr.ts1"
        sfr.ts.initialize(
            filename=fname1,
            timeseries=ts1_data,
            time_series_namerecord=ts1_names,
            interpolation_methodrecord=ts1_methods,
        )

        fname2 = f"{name}.sfr.ts2"
        sfr.ts.append_package(
            filename=fname2,
            timeseries=ts2_data,
            time_series_namerecord=ts2_names,
            interpolation_methodrecord=ts2_methods,
        )
...
def build_models(idx, test):
    ...
    ws = os.path.join(test.workspace)
    sim = get_model(ws, name, timeseries=True)
...
def check_result(idx, test):
    name = cases[idx]
    fname1 = f"{name}.sfr.ts1"
    fpth = os.path.join(test.workspace, fname1)

    assert os.path.isfile(fpth), "first time series file unexpectedly missing"

    fname2 = f"{name}.sfr.ts2"
    assert os.path.isfile(os.path.join(test.workspace, fname2)), (
        "second time series file unexpectedly missing"
    )
