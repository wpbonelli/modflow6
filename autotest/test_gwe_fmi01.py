"""
Tests that a GWE model can read the GWF model's binary grid
file via the FMI package's GWFGRID entry.

Previously GWE treated GWFGRID as the name of an advanced
...
The second case gives the GWE model an IDOMAIN that differs
...
porosity = 0.1
cpw, rhow = 4180.0, 1000.0
cps, rhos = 880.0, 2500.0
...
def build_gwe_sim(name, gwf_ws, gwe_ws, mf6):
    gwf_name = get_model_name(name, "gwf")
    gwe_name = get_model_name(name, "gwe")
    ...
    flopy.mf6.ModflowGweoc(
        gwe,
        budget_filerecord=f"{gwe_name}.bud",
...
def build_models(idx, test):
    ...
    gwe_ws = test.workspace / "gwe"
    gwf_sim = build_gwf_sim(test.name, gwf_ws, test.targets["mf6"])
    gwe_sim = build_gwe_sim(test.name, gwf_ws, gwe_ws, test.targets["mf6"])
    return gwf_sim, gwe_sim
...
def check_output(idx, test):
    ...
    gwe_name = get_model_name(test.name, "gwe")
    fpth = test.workspace / "gwe" / f"{gwe_name}.ucn"
    temp = flopy.utils.HeadFile(
        fpth, precision="double", text="TEMPERATURE"
    ).get_alldata()

    # heat should have entered the domain and stay bounded
    assert temp[-1].max() > 0.0
    assert np.all(temp >= 0.0)
    assert np.all(temp <= 1.0 + 1.0e-6)
