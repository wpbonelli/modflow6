...
def check_output(idx, test, snapshot):
    ...
    if "drape" in name:
        ...
        # expect a water table event for each particle when the water
        # rises and entrains it, then 2 more when the water recedes
        # below 2 of the particles near the end of the simulation
        assert len(actual[actual.ireason == 6]) == nparts + 2

        # expect a user time event for each particle when the user time is reached
