...
def get_discharge(
    ...
    roughness: Union[int, float, np.ndarray] = 0.01,
    slope: float = 0.001,
    conv: float = 1.0,
):
    """
    ...
    if isinstance(x, int) or isinstance(x, float):
        ...
        if not (isinstance(roughness, int) or isinstance(roughness, float)):
            raise ValueError("Roughness must be an integer or float")
        ...
        if len(x) < 1:
            raise ValueError(f"Arrays x and h must have at least 1 element")
        ...
        if len(x) == 1:
            ...
            q = get_discharge_rect(x[0], depth, roughness[0], slope, conv)
        else:
            q = 0.0
...
def get_depth(
    ...
    Channel geometry is given as arrays of x-coordinates and elevations. The
    arrays must have the same length. A single roughness coefficient may be
    provided, or distinct values for each segment. The function iteratively
    refines an estimate of depth of flow until the error is below a provided
    precision or a maximum number of iterations is reached (default is 100).
    ...
    precision: the error threshold
    max_iter: the maximum number of iterations
    verbose: whether to print residual at each iteration

    Returns
    -------
        The estimated flow depth.
    """
...
def get_depths(
    flows: Union[int, float, np.ndarray],
    x: Union[int, float, np.ndarray],
    h: Union[int, float, np.ndarray],
    ...
    given its cross-sectional geometry, a roughness coefficient, the rate of
    flow, and the channel slope. An optional conversion factor may be given.
    For English units specify 1.49 for the conversion factor. A single flow
    rate can be provided, or an array of flow rates. The return value is an
    array containing estimated depth for each flow rate, in the order given.
    Channel geometry is given as arrays of x-coordinates and elevations. The
    arrays must have the same length. A single roughness coefficient may be
    provided, or distinct values for each segment. The function iteratively
    refines an estimate of depth of flow until the error is below a provided
    ...
    Returns
    -------
        An array of estimated flow depths (one for each flow rate provided).
    """

    if isinstance(flows, float):
        flows = np.array([flows], dtype=float)
    if isinstance(x, np.ndarray) and isinstance(roughness, float):
