...
splitting the shared boundary, both in separate grids via an exchange,
and in the same grid.

Three test cases are included:
1. Basic GWF-GWF exchange (without interface model)
...
cases = [
    "gwfgwf_disv_split_face",
    "gwfgwf_disv_split_face_ifmod",
    "gwf_disv_split_face",
]
...
def check_output(idx, test):
    if idx == 2:
        ...
        # exchange cases
        fpth1 = test.workspace / "gwf1.hds"
        fpth2 = test.workspace / "gwf2.hds"

        hds1 = flopy.utils.HeadFile(fpth1)
        hds2 = flopy.utils.HeadFile(fpth2)

        head1 = hds1.get_data()
        head2 = hds2.get_data()

        assert 0.9 < head1[0, 0] < 1.1
        assert -0.1 < head2[0, 0] < 0.1
        assert head1[0, 0] > head2[0, 0]
