An iterative model solution (IMS) is specified within the SOLUTIONGROUP block in the simulation name file.  The model solution will solve all of the numerical models that are added to it, as specified in the simulation name file, and will include numerical exchanges, if they are present.  Numerical models require a numerical solution procedure, and include for example the Groundwater Flow (GWF), Groundwater Transport (GWT) and Groundwater Energy Transport (GWE) Models.  The iterative model solution requires specification of both nonlinear and linear settings.  
