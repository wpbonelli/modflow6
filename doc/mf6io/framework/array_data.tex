...
\item \texttt{(BINARY)}---is an option that indicates the OPEN/CLOSE file contains array data in binary (unformatted) form.  A binary file that can be read by MODFLOW requires special handling.  Information on the binary format of the binary file is discussed later in this section.
